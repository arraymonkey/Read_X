...
function periodicSaveIfNeeded() {
  try {
    ...
    //don't save scroll location because the client window is not visible and the scroll info is not valid
    if( myCodeMirror?.getScrollInfo()?.clientHeight < 0 ){
      return;
    }
    

    let str = scan();
    if (lastScan != "" && lastScan != str) {
      save_options();
    }
    lastScan = str;
  } catch (ex) {
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  ...
  if (message && message != undefined && message.match(/readx replytoextramessage/gim)) {
    let subject = request.data.subject;
    //console.log(subject);
    document.getElementById("scanresults").value += subject.substring("readx replytoextramessage".length) + "\n";
  }
});

function extraSendMessage() {

  let id = "<id:>" + document.getElementById("handleid").value;
  let words = "<words:>" + document.getElementById("startingwords").value;

  if( id.trim() == "<id:>" ){
    document.getElementById("scanresults").value = "";
  }

  sendMessageToBackground("readx extramessage " + id + " " + words);
}
...
function callback(tabs) {
  ...
  let myUrl = currentTab.url;
  var regEx = new RegExp("\n?" + regexEscape(myUrl), "ig");

  let listVal = document.getElementById("blacklist").value.trim();
  listVal = listVal.replace(regEx, "");
  listVal += "\n" + myUrl;
  listVal = listVal.trim();
  document.getElementById("blacklist").value = listVal;
...
function updateClicktype(checked) {
  let clicktype = document.getElementById("clicktype");
  if (checked) {
    clicktype.style.display = "initial";
  } else {
...
function restore_options() {
  ...
  document.getElementById("third").addEventListener("click", openTab);
  document.getElementById("fourth").addEventListener("click", openTab);
  ...
  document.getElementById("btnblacklist").addEventListener("click", getTabUrl);

  document.getElementById("btnSendMessage").addEventListener("click", extraSendMessage);
...
function loadItems(items) {
  ...
  updateClicktype(items.clickValue);
...
function addListeners() {
  ...
  document.getElementById("click").addEventListener("change", function (e) {
    updateClicktype(e.target.checked);
    saveOnChange();
  });
